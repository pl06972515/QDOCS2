...
| 关系运算符                                | <span style='color:red'>[`==`宽松相等：如果两个操作数类型不同，`JS`会尝试将它们转换为相同类型再比较 ] </span>`!=  >  >=  <  <=` |
...
console.log(false == 0)        // true (false被转换为0)
console.log(null == undefined) // true
console.log("" == 0)           // true (空字符串被转换为0)

// === 示例（严格比较）
console.log(5 === "5")          // false (数字5不等于字符串"5")
console.log(true === 1)         // false (布尔值不等于数字)
